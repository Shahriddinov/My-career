...
function ModalCom({FrlExpDescrHandler, FrlExpDescr, FrlExpStartHandler, FrlExpStart, FrlExpEndHandler, FrlExpEnd, FrlCurrentWorkHandler, FrlCurrentWork, FrlExperienceCompJobHandler, FrlExperienceCompJob, FrlExperienceCompNameHandler, FrlExperienceCompName}) {
    ...
    return (
        <>
                         ...
                         experience.map((item)=>(
                           ...
                           <div className="w-75">
                               ...
                               <div className="modal-image-e-d">
                                   <img src="image/edit (1).png" alt="" onClick={()=>editTask(item.id)}/>
            ...
            <Modal isOpen={modal} toggle={toggle}>
                ...
                <ModalBody>
                    <input onChange={(e) => FrlExperienceCompNameHandler(e.target.value)} placeholder="Company name"
                           type="text" className="form-control mt-4 inputs-all1"/>
                    <input onChange={(e) => FrlExperienceCompJobHandler( e.target.value)} placeholder="Job" type="text"
                           className="form-control mt-4 inputs-all1"/>
                    <input onChange={(e) => FrlCurrentWorkHandler(!FrlCurrentWork)} className="checkbox-in mt-4"
                           type="checkbox"/>
                    ...
                    <div className="all-r-l-input">
                        <div className="right-input">
                            ...
                            <input onChange={(e)=>FrlExpStartHandler(e.target.value)} type="date" className="form-control inputs-all"/>
                        </div>
                        <div className="left-input">
                            <div>
                                <label className="mt-4 select-curr" htmlFor="">to</label>
                                <input onChange={(e)=>FrlExpEndHandler(e.target.value)} type="date" className="form-control inputs-all"/>
                            </div>
                        </div>
                    </div>
                    <textarea onChange={(e)=>FrlExpDescrHandler(e.target.value)} className="form-control mt-3" name="" id="" cols="30" rows="3"></textarea>
                </ModalBody>
                <ModalFooter>
