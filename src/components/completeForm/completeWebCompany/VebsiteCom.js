...
function VebsiteCompany(props) {
    ...
    const { CompanyWebsiteHendler,CompanyWhatsappHendler,CompanyFacebookHendler,CompanyInstagramHendler,CompanyTelegramHendler,CompanyGithubHendler,CompanyTwitterHendler } = useStore()
    ...
    return (
        <>
           ...
           <div>
               ...
               <input onChange={(e)=>CompanyWebsiteHendler(e.target.value)} type="text" className="form-control inputs-all1 websiteInput mt-4" placeholder="Provide a link to your website "/>
           </div>
           {
               arr.map((item,i) => {
                   return item.isActive === false ?  <VebsiteComponent key={i} i={i} ValueHendler={ValueHendler} contactHendler1={contactHendler1} item={item} /> : ""
               })
           }
